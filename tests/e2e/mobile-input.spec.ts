...
test('탁구와 배구는 양쪽 pointer hold를 동시에 처리하고 모든 종료 경로에서 해제한다', async ({
  ...
  for (const game of ['pong', 'volleyball'] as const) {
    ...
    if (game === 'pong') {
      expect(firstAfterKeyRelease.y).toBeLessThan(firstBefore.y - 4);
      expect(secondAfter.y).toBeGreaterThan(secondBefore.y + 4);
    } else {
      expect(firstAfterKeyRelease.x).toBeLessThan(firstBefore.x - 4);
      expect(secondAfter.y).toBeLessThan(secondBefore.y - 4);
    }
