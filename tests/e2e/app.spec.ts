...
test('로비는 데스크톱과 모바일에서 오버플로 없이 로드된다', async ({ page }) => {
  ...
  for (const viewport of [
    ...
    await expect(page.locator('.app-nav [data-action="lobby"]')).toHaveAttribute(
      'aria-current',
      'page',
    );
    const aboutNavigation = page.locator('.app-nav a[href="/about/"]');
    const fairnessNavigation = page.locator('.app-nav a[href="/fairness/"]');
    await expect(aboutNavigation).toHaveCount(1);
    await expect(fairnessNavigation).toHaveCount(1);
    if (viewport.width <= 560) {
      await expect(aboutNavigation).toBeHidden();
      await expect(fairnessNavigation).toBeHidden();
    } else {
      await expect(aboutNavigation).toBeVisible();
      await expect(fairnessNavigation).toBeVisible();
    }
    await expect(page.locator('.site-footer__nav a[href="/about/"]')).toHaveCount(1);
...
test('모든 게임 카드가 열리고 로비로 돌아온다', async ({ page }) => {
  const errors = collectPageErrors(page);
  await page.goto('./play/#lobby');
  const cleanRootUrl = new URL('../', page.url());
  for (const gameId of games) {
    const startButton = page.locator(`button.game-card__start[data-game-id="${gameId}"]`);
    const card = page.locator('article.game-card').filter({ has: startButton });
    await expect(card.locator('a.game-card__guide')).toHaveAttribute(
      'href',
      new URL(`games/${gameId}/`, cleanRootUrl).pathname,
    );
...
test('SPA 화면 전환 뒤 키보드 포커스가 새 화면 제목으로 이동한다', async ({ page }) => {
  await page.goto('./play/#lobby');
  const omokStart = page.locator('button.game-card__start[data-game-id="omok"]');
  await expect(omokStart).toHaveAccessibleName('오목 시작');
  await omokStart.focus();
  await page.keyboard.press('Enter');
  await expect(page.locator('#game-host')).toHaveAttribute('aria-busy', 'false');
  await expect(page.locator('#stage-title')).toBeFocused();
  await expect(page.locator('.app-nav [aria-current]')).toHaveCount(0);
...
test('게임 화면에서 바꾼 이름은 다음 시작 때 모듈 UI에도 반영된다', async ({ page }) => {
  ...
  await page.goto('./play/#game/tap-battle');
  await expect(page.locator('#game-host')).toHaveAttribute('aria-busy', 'false');
  ...
  await page.goto('./play/#game/reaction-duel');
  await expect(page.locator('#game-host')).toHaveAttribute('aria-busy', 'false');
  ...
  await page.goto('./play/#game/volleyball');
  await expect(page.locator('#game-host')).toHaveAttribute('aria-busy', 'false');
...
test('로비와 게임 공통 화면에 심각한 axe 위반이 없다', async ({ page }) => {
  for (const path of ['./play/#lobby', './play/#game/omok']) {
...
test('공유 버튼은 로비 root와 게임별 clean 가이드 URL을 전달한다', async ({ page }) => {
  ...
  await page.goto('./play/#lobby');
  const playCanonical = await page.locator('link[rel="canonical"]').getAttribute('href');
  expect(playCanonical).toBeTruthy();
  const cleanRootUrl = new URL('../', playCanonical ?? '').href;
  const sharedUrl = (): Promise<string | null> =>
    page.evaluate(() => sessionStorage.getItem('moyeoplay-test-share-url'));

  await page.getByRole('button', { name: '링크 공유', exact: true }).click();
  await expect.poll(sharedUrl).toBe(cleanRootUrl);

  await page.goto('./play/#game/pong');
  await expect(page.locator('#game-host')).toHaveAttribute('aria-busy', 'false');
  await page.getByRole('button', { name: '현재 게임 가이드 공유' }).click();
  await expect.poll(sharedUrl).toBe(new URL('games/pong/', cleanRootUrl).href);
});
