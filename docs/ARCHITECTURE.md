...
`design/game-icons/source/<id>.png`의 RGBA master를 Sharp가 320×320 AVIF·WebP·PNG로 만든다. `scripts/build-brand-assets.mjs`는 현재 파란 M SVG에서 favicon·180/192/512/maskable icon을 만들고, 밝은 클레이 템플릿과 게임 PNG를 1200×630 OG로 합성한다. 카드 `<picture>`는 고정 크기를 예약하고 이미지 로드 성공 전까지 기존 glyph를 fallback으로 유지한다. 게임 가이드의 1280×720 WebP는 Playwright가 고정 viewport·난수·모션 설정으로 실제 `/play/` 런타임을 캡처한다.
...
GitHub Actions custom workflow에서는 artifact의 `CNAME`이 Pages custom domain을 설정하지 않는다. Settings → Pages, DNS, custom TLS와 Enforce HTTPS는 build artifact 바깥의 운영 상태다. 2026-07-21 재검증에서 DNS, 승인된 apex·`www` 인증서, HTTPS enforcement와 redirect 출시 게이트가 통과했다.
