...
- 2026-07-21 현재 custom-domain TLS 인증서와 HTTPS enforcement, HTTP·`www`·이전 Pages URL redirect가 live에서 확인됐다.
...
| custom hostname TLS        | 확인 완료 |
| Enforce HTTPS              | 확인 완료 |
| HTTP→HTTPS path redirect   | 확인 완료 |
| secure `www`→apex redirect | 확인 완료 |

권한 DNS는 Name.com이고 HTTPS 종료·인증서 자동 갱신은 GitHub Pages가 담당한다. Name.com Security Center의 별도 SSL 상품이 비활성이어도 실제 Pages HTTPS가 활성 상태이므로 중복 유료 인증서는 필요하지 않다. 앞으로도 DNS만 맞는 상태를 HTTPS 완료로 보고하지 않고 실제 인증서와 redirect를 함께 확인한다.
...
- [x] custom-domain TLS·HTTPS enforcement·redirect가 live에서 통과했다.
...
HTTPS 출시 게이트는 통과했지만 운영자·문의 정보, Search Console, AdSense Sites 검증·심사 결과, 실제 인증 CMP와 광고 배치 검토가 남아 있으므로 수익화 출시 승인 상태는 아니다.
