...
- 작업 경로에 `.git`과 원격 정보가 없고 `gh` CLI도 없다. 실제 소유자/저장소명, Pages base, 공개 URL은 임의로 조작하지 않고 배포 단계에서 검증한다.
...
- **사다리**: 2~8명 안전한 입력, 시드 재현, 겹치지 않는 가로선, 밀도/전체 gap 보장, 1:1 순열, 경로 애니메이션, 전체 결과/복사.
- **반응속도**: 예측하기 어려운 지연, 부정 출발, 키/화면 반쪽 멀티터치, 반응 시간, 5판 3선승.
...
- Vitest로 오목 승리/무승부/무르기, 사다리 permutation/가로선/시드, 탁구 반사/속도/점수 latch, 배구 바닥 latch/충돌, 핀볼 대칭/착지/무승부/부스터, 저장소 마이그레이션/손상, 룰렛 인덱스/각도를 테스트한다.
...
- 현재 원격을 알 수 없으므로 base는 우선 `./`과 hash routing으로 임의 프로젝트 하위 경로에서도 작동하게 한다. 원격이 복구되면 `<owner>.github.io`/커스텀 도메인은 `/`, 프로젝트 Pages는 `/<repo>/`로 확정한다.
- canonical, Open Graph, sitemap은 확인되지 않은 URL을 만들지 않는다. 배포 URL이 확정된 시점에 최종 대체하고 공개 URL 200, CSS/JS, Canvas 시작, `#game/*`, 모바일을 실제 검증한다.
